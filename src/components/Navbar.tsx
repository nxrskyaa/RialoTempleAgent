...
export default function Navbar() {
  ...
  return (
    <nav className="fixed left-0 right-0 top-0 z-50 border-b border-[var(--temple-border)] bg-[#071014]/84 backdrop-blur-xl">
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between gap-2 px-3 sm:px-6">
        <Link to="/" className="flex min-w-0 items-center gap-2">
          <span className="relative flex h-10 w-10 items-center justify-center rounded-lg border border-[var(--temple-border)] bg-white/[0.055] shadow-[0_0_28px_rgba(108,242,184,0.12)]">
            <img src="/logo-mark.svg" alt="" className="h-8 w-8" />
          </span>
          <span className="temple-wordmark hidden text-base font-semibold text-[var(--temple-text)] sm:inline">Rialo Temple</span>
        </Link>
        <div className="flex items-center gap-1 rounded-lg border border-[var(--temple-border)] bg-white/[0.045] p-1">
          {NAV.map(item => {
            ...
            return (
              <Link key={item.path} to={item.path}
                ...
                style={{ color: active ? '#071014' : 'var(--temple-muted)' }}>
                {active && <motion.div layoutId="nav" className="absolute inset-0 rounded-md bg-[linear-gradient(135deg,#6cf2b8,#ffe27a,#ff8f83)]" transition={{ type: 'spring', stiffness: 500, damping: 35 }} />}
