  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-[var(--temple-gold)]">Leaderboard</p>
          <h1 className="mt-2 text-4xl font-black tracking-normal">Temple rankings</h1>
          <p className="mt-2 text-sm text-[var(--temple-muted)]">Ranked by total PTS, then best streak, then total check-ins.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="temple-card rounded-lg py-20 text-center">
        ...
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="temple-card rounded-lg p-5">
            <p className="mb-5 text-xs font-semibold uppercase tracking-wider text-[var(--temple-soft)]">Podium</p>
            <div className="grid grid-cols-3 items-end gap-3">
              {[entries[1], entries[0], entries[2]].filter(Boolean).map((entry, index) => {
                ...
                return (
                  <div key={entry.user} className={`rounded-lg border border-[var(--temple-border)] bg-black/20 p-3 text-center ${rank === 1 ? 'pb-8 pt-7' : index === 0 ? 'pb-5 pt-5' : 'pb-4 pt-4'}`}>
                    <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center overflow-hidden rounded-lg bg-white/[0.04]">
                    ...
                    <p className="mt-1 truncate text-sm font-semibold">{entry.name || fmtAddress(entry.user)}</p>
          ...
          <div className="temple-card rounded-lg p-5">
            <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-[var(--temple-soft)]">All builders</p>
            <div className="space-y-2">
              {entries.map((entry, index) => {
                const tier = getTier(entry.currentStreak)
                return (
                  <motion.div key={entry.user} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.025 }} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-lg border border-[var(--temple-border)] bg-black/20 p-3">
                    <span className="w-8 text-center text-sm font-black" style={{ color: index < 3 ? tier.color : 'var(--temple-soft)' }}>#{index + 1}</span>
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <div className="h-8 w-8 overflow-hidden rounded-lg bg-white/[0.04]">
                          {entry.avatarUrl && <img src={entry.avatarUrl} alt="" className="h-full w-full object-cover" />}
                        </div>
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold">{entry.name || fmtAddress(entry.user)}</p>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="temple-card rounded-lg p-4">
