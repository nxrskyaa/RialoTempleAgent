  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <div className="grid gap-5 lg:grid-cols-[1fr_0.72fr]">
        <section className="temple-rune-panel temple-card spark-field overflow-hidden rounded-lg p-5 sm:p-7">
          <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div className="max-w-2xl">
              <p className="text-xs font-black uppercase tracking-wider text-[var(--temple-cyan)]">Grialo daily ritual</p>
              <h1 className="arcade-title mt-2 text-4xl font-black tracking-normal sm:text-5xl">Feed the little flame</h1>
              <p className="mt-3 max-w-xl text-sm leading-6 text-[var(--temple-muted)]">Welcome, {profileName}. One on-chain tap every 24 hours keeps the streak warm and the leaderboard noisy.</p>
            </div>
            <div className="sticker-coral flex shrink-0 items-center gap-3 rounded-lg px-4 py-3">
          ...
          <div className="relative z-10 mt-8 grid gap-4 md:grid-cols-[0.95fr_1.05fr]">
            <div className="machine-frame relative flex min-h-[340px] items-center justify-center rounded-lg p-5 text-center">
              <div className="temple-orbit absolute h-64 w-64 rounded-full p-[1px]" />
              <div className="machine-screen absolute flex h-48 w-48 items-center justify-center rounded-lg">
                <div className="flame-buddy h-24 w-20 opacity-90" />
              </div>
              <div className="relative z-10">
                <p className="text-xs font-black uppercase tracking-wider text-[var(--temple-gold)]">Current streak</p>
                <div className="mt-4 text-8xl font-black leading-none" style={{ color: tier.color }}>{stats.currentStreak}</div>
                <p className="mt-2 text-sm font-black">{tier.name}</p>
                <p className="text-xs text-[var(--temple-muted)]">{tier.range}</p>
              </div>
            </div>

            <div className="pixel-panel flex flex-col justify-between rounded-lg border border-[var(--temple-border)] p-5">
              <AnimatePresence mode="wait">
                {success ? (
                  <motion.div key="success" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="flex flex-1 flex-col items-center justify-center text-center">
                    ...
                    <h2 className="text-2xl font-black">Ritual confirmed</h2>
                  ...
                  <motion.div key="ready" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                    <p className="text-xs font-black uppercase tracking-wider text-[var(--temple-soft)]">Transaction</p>
                    <h2 className="mt-2 text-2xl font-black">1 native USDC</h2>
        ...
        <aside className="space-y-4">
          <div className="temple-card rounded-lg p-5">
            <p className="mb-4 text-xs font-black uppercase tracking-wider text-[var(--temple-soft)]">Your stats</p>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={label} className="pixel-panel rounded-lg border border-[var(--temple-border)] p-4">
          ...
          <div className="temple-card rounded-lg p-5">
            <div className="mb-4 flex items-center justify-between">
              <p className="text-xs font-black uppercase tracking-wider text-[var(--temple-soft)]">Tier ladder</p>
